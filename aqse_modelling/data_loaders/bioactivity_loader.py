...
class BioactivityDataLoader:
    ...
    def __init__(self, config):
        ...
        self.esmc_cache_dir = Path(
            config.get("papyrus_cache_dir", output_dir / "esmc_cache")
        )
        ...
        self.proteins_use_standard_papyrus = config.get(
            "proteins_use_standard_papyrus", []
        )
        if self.proteins_use_standard_papyrus:
            self.logger.info(
                f"Proteins using standard Papyrus: {self.proteins_use_standard_papyrus}"
            )
        ...
        try:
            ...
            self.logger.info(
                f"Loaded {len(self.papyrus_df):,} total activities from Papyrus++"
            )
        except Exception as e:
    ...
    def calculate_morgan_fingerprints(
        self, unique_proteins: pl.DataFrame
    ) -> Dict[str, Any]:
        """
        Step 0: Calculate and save Morgan fingerprints for all compounds in Papyrus database,
        only for the compounds associated with the unique protein list
        ...
        try:
            ...
            valid_data = valid_data.filter(pl.col("accession").is_in(list(unique_ids)))
            self.logger.info(
                f"{len(valid_data)} activities after filtering by unique proteins"
            )
            ...
            # Save fingerprints as Parquet for efficiency
            fingerprints_file = (
                self.fingerprints_dir / "papyrus_morgan_fingerprints.parquet"
            )
    ...
    def load_morgan_fingerprints(
        self, unique_proteins: pl.DataFrame | None = None
    ) -> Dict[str, np.ndarray]:
        """Load pre-computed Morgan fingerprints
        ...
        """
        # Try Parquet first, fallback to pickle for backward compatibility
        fingerprints_file_parquet = (
            self.fingerprints_dir / "papyrus_morgan_fingerprints.parquet"
        )
        fingerprints_file_pkl = (
            self.fingerprints_dir / "papyrus_morgan_fingerprints.pkl"
        )
        ...
        try:
            ...
            if fingerprints_file_parquet.exists():
                ...
                self.logger.info(
                    f"Loaded {len(fingerprints)} Morgan fingerprints from Parquet"
                )
                return fingerprints
    ...
    def calculate_physicochemical_descriptors(self, smiles: str) -> Dict[str, float]:
        ...
        try:
            ...
            self.logger.error(
                f"Error calculating physicochemical descriptors for {smiles}: {e}"
            )
            return {}
    ...
    def _load_standard_papyrus(self):
        ...
        try:
            ...
            self.logger.info(
                f"Loaded {len(self.papyrus_standard_df):,} total activities from standard Papyrus"
            )
        except Exception as e:
        ...
        try:
            ...
            for target in targets:
                ...
                if name and uniprot_id:
                    ...
                    # Also map UniProt ID to itself for direct lookups
                    self._protein_name_to_uniprot[uniprot_id.upper()] = (
                        uniprot_id.upper()
                    )

            if self._protein_name_to_uniprot:
                self.logger.debug(
                    f"Built protein name mapping for {len(self._protein_name_to_uniprot)} entries"
                )
        except Exception as e:
    ...
    def get_filtered_papyrus(
        self, uniprot_ids: List[str] | None = None
    ) -> pl.DataFrame:
        """
        Get filtered Papyrus data for specific proteins.
        Automatically selects Papyrus++ or standard Papyrus based on config.
        ...
        # Check if any of the requested proteins need standard Papyrus
        needs_standard = any(
            self._should_use_standard_papyrus(uid) for uid in uniprot_ids
        )
        needs_plusplus = any(
            not self._should_use_standard_papyrus(uid) for uid in uniprot_ids
        )
        ...
        if needs_standard:
            ...
            standard_ids = [
                uid for uid in uniprot_ids if self._should_use_standard_papyrus(uid)
            ]
            if standard_ids and not self.papyrus_standard_df.is_empty():
                standard_df = self.papyrus_standard_df.filter(
                    pl.col("accession").is_in(standard_ids)
                )
                if not standard_df.is_empty():
                    self.logger.info(
        ...
        if needs_plusplus:
            if self.papyrus_df.is_empty():
                ...
                plusplus_ids = [
                    uid
                    for uid in uniprot_ids
                    if not self._should_use_standard_papyrus(uid)
                ]
                if plusplus_ids:
                    plusplus_df = self.papyrus_df.filter(
                        pl.col("accession").is_in(plusplus_ids)
                    )
                    if not plusplus_df.is_empty():
                        if needs_standard:
                            self.logger.info(
    ...
    def load_esmc_descriptors(self, uniprot_id: str) -> Optional[np.ndarray]:
        ...
        try:
            ...
            if isinstance(esmc_data, dict):
                ...
                # Handle DataFrame format
                esm_cols = [
                    col for col in esmc_data.columns if col.startswith("esm_dim_")
                ]
