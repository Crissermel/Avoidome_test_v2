...
def _load_thresholds(activity_thresholds_file) -> pl.DataFrame:
    """Load optimized activity thresholds from CSV file"""
    try:
        logger.info(
            f"Loading optimized activity thresholds from {activity_thresholds_file}"
        )
        df = pl.read_csv(activity_thresholds_file)
        ...
        if "optimized_cutoff_high" in df.columns:
            ...
            logger.info("Using optimized high cutoffs")
        if "optimized_cutoff_medium" in df.columns:
            df = df.with_columns(
                pl.col("optimized_cutoff_medium").alias("cutoff_medium")
            )
            logger.info("Using optimized medium cutoffs")

        # Ensure use_2class column exists (for backward compatibility)
        if "use_2class" not in df.columns:
            logger.warning(
                "'use_2class' column not found in thresholds file. All proteins will use 3-class classification."
            )
            df = df.with_columns(pl.lit(False).alias("use_2class"))
        else:
            # Convert use_2class to boolean if it's stored as string
            df = df.with_columns(
                pl.col("use_2class").cast(pl.Boolean).alias("use_2class")
            )
        ...
        logger.info(f"Loaded activity thresholds for {len(df)} proteins")

        # Count 2-class proteins
        n_2class = df["use_2class"].sum() if "use_2class" in df.columns else 0
        logger.info(f"Found {n_2class} proteins recommended for 2-class classification")
        return df
    except Exception as e:
        logger.error(f"Error loading activity thresholds: {e}")
        return pl.DataFrame()
...
def get_thresholds(
    ...
    """
    if thresholds_df.is_empty():
        return {}

    try:
        # Look up by UniProt ID
        protein = thresholds_df.filter(pl.col("uniprot_id") == uniprot_id)

        if protein.is_empty():
            logger.warning(f"No thresholds found for {uniprot_id}")
            return {}
        ...
        if "use_2class" not in row and uniprot_id in two_class_proteins:
            use_2class = True
            logger.warning(
                f"Using hardcoded 2-class list for {uniprot_id} (CSV missing use_2class column)"
            )
        ...
        if use_2class:
            ...
            logger.info(
                f"Using 2-class classification for {uniprot_id}: {best_2class_option} (cutoff={cutoff_to_use:.1f})"
            )
            return {
                "high": cutoff_high,
                "medium": float(row.get("cutoff_medium", cutoff_high))
                if best_2class_cutoff == "medium"
                else None,
                "n_classes": 2,
        ...
        logger.error(f"Error getting thresholds for {uniprot_id}: {e}")
        return {}


def get_thresholds_by_name(
    thresholds_df: pl.DataFrame, protein_name: str
) -> Dict[str, Any]:
    """
    ...
    try:
        protein = thresholds_df.filter(pl.col("name2_entry") == protein_name)

        if protein.is_empty():
            logger.warning(f"No thresholds found for {protein_name}")
            return {}

        row = protein.row(0, named=True)
        uniprot_id = row["uniprot_id"]
        # Use get_thresholds to get the full dict with n_classes
        return get_thresholds(thresholds_df, uniprot_id)
    except Exception as e:
        logger.error(f"Error getting thresholds for {protein_name}: {e}")
        return {}


def get_all_proteins(thresholds_df) -> List[str]:
    """Get list of all proteins with thresholds"""
    if thresholds_df.is_empty():
        return []
    return thresholds_df["uniprot_id"].drop_nulls().to_list()
...
class ActivityThresholdsLoader:
    ...
    def _load_thresholds(self):
        ...
        try:
            ...
            if "optimized_cutoff_high" in df.columns:
                df = df.with_columns(
                    pl.col("optimized_cutoff_high").alias("cutoff_high")
                )
            ...
            n_2class = df["use_2class"].sum() if "use_2class" in df.columns else 0
            self.logger.info(
                f"Found {n_2class} proteins recommended for 2-class classification"
            )
        except Exception as e:
    ...
    def _load_2class_proteins(self):
        ...
        # This method is kept for backward compatibility but is no longer used.
        # The use_2class flag is now read directly from the thresholds CSV file.
        self.two_class_proteins = (
            set()
        )  # Empty set - will be populated from CSV if needed
